 ...
 <div class="container" >
    ...
    <div class="dgvone">
       <table class="table table-condensed table-bordered table-striped" id="tableid">
          ...
          <s:set var="cc" value="%{#allannounces.cont}"></s:set>  
	          ...
	          <table>
    <form action="${pageContext.request.contextPath }/announcesAction!queryAnnouncesByGroupname.go?groupname=${allannounces.groupname}" method="post" style="float: left;">
	    <tr>
    ...
    <div class="STYLE9"">
      <td  width="12" align="center">页次:${currentPage}/${sumPage}页&nbsp;每页${pageSize}条信息共${count}条
        ...
        <a class="STYLE9" href="${pageContext.request.contextPath}/announcesAction!getAnnouncesByGroupname_all1.go?currentPage=1&&groupname=${allannounces.groupname}">首页</a>
		
		<s:if test="currentPage==1">上一页</s:if>
		
		
		<s:else>
		<a class="STYLE9" href="${pageContext.request.contextPath}/announcesAction!getAnnouncesByGroupname_all1.go?currentPage=${currentPage-1}&&groupname=${allannounces.groupname}">上一页</a>
		</s:else>
		
		<s:if test="currentPage>=sumPage">下一页</s:if>
		
		<s:else>
		<a class="STYLE9" href="${pageContext.request.contextPath}/announcesAction!getAnnouncesByGroupname_all1.go?currentPage=${currentPage+1}&&groupname=${allannounces.groupname}">下一页</a>
		</s:else>
		
		
		<a class="STYLE9" href="${pageContext.request.contextPath}/announcesAction!getAnnouncesByGroupname_all1.go?currentPage=${sumPage}&&groupname=${allannounces.groupname}">尾页</a></td>
    </div>
  </div>
