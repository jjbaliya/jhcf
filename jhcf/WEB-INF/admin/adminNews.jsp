...
<!--   <link rel="stylesheet" href="${ctx}/css/roomset/roomset.css" type="text/css"></link> -->
 ...
 <div class="container" >
    <table>
    <form action="${pageContext.request.contextPath }/newsAction!queryNews.go" method="post" style="float: left;">
	    <tr>
	         ...
	         <s:iterator var="allnews" value="listNews">
	           <s:set var="cc" value="%{#allnews.cont}"></s:set>
		        <tr>
		          ...
		          <td><a class="STYLE4" href="${pageContext.request.contextPath }/newsAction!getNewsById.go?news.id=${allnews.id}">
		          <img src="images/edt.gif" width="16" height="16" />编辑</a>&nbsp; &nbsp;
		          <a class="STYLE4" onClick="{if(confirm('此操作将删除当前记录!删除后将不能恢复,您确定执行的操作吗?')){return true;}return false;}" href="${pageContext.request.contextPath }/newsAction!deleteNews.go?news.id=${allnews.id}"><img src="images/del.gif" width="16" height="16" />删除</a></td>
    ...
    <div class="STYLE9"">
      <td  width="12" align="center">页次:${currentPage}/${sumPage}页&nbsp;每页${pageSize}条信息共${count}条
        ...
        <a class="STYLE9" href="${pageContext.request.contextPath}/newsAction!getAllNews.go?currentPage=1">首页</a>
		
		<s:if test="currentPage==1">上一页</s:if>
		
		
		<s:else>
		<a class="STYLE9" href="${pageContext.request.contextPath }/newsAction!getAllNews.go?currentPage=${currentPage-1}">上一页</a>
		</s:else>
		
		<s:if test="currentPage>=sumPage">下一页</s:if>
		
		<s:else>
		<a class="STYLE9" href="${pageContext.request.contextPath }/newsAction!getAllNews.go?currentPage=${currentPage+1}">下一页</a>
		</s:else>
		
		
		<a class="STYLE9" href="${pageContext.request.contextPath }/newsAction!getAllNews.go?currentPage=${sumPage}">尾页</a></td>
    </div>
  </div>
